...
export default function SuperPerformanceManagement({ stock, klines }: SuperPerformanceManagementProps) {
  const { buyPoint, lastClose, stopLoss, pivotCreationDate, lastMA50 } = stock;
  ...
  const stats = useMemo(() => {
    ...
    const isBelowMA50 = lastMA50 ? lastClose < lastMA50 : false;
  ...
  }, [stock, klines, pivotCreationDate, lastClose, buyPoint, stopLoss, lastMA50]);
  ...
  const showBreakoutInfo = stock.pivotStatus === 'Breakout' || stock.pivotStatus === 'Pullback';
  ...
  return (
    <div className="bg-[#161B22] border border-[#30363D] rounded-xl p-5 shadow-sm space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800/50">
               <span className="text-[11px] text-gray-500 font-bold flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> {showBreakoutInfo ? "突破日點位 / 日期" : "Pivot 壓力位 / 紀錄"}</span>
            ...
            <div className="flex items-center justify-between pb-2 border-b border-slate-800/50">
               <span className="text-[11px] text-gray-500 font-bold flex items-center gap-1.5"><TrendingUp className="w-3.5 h-3.5" /> 追蹤交易日數</span>
               <span className="text-xs font-mono font-black text-gray-200">{stats.daysSinceBreakout} 天</span>
            </div>

            <div className="flex items-center justify-between pb-2 border-b border-slate-800/50">
               <span className="text-[11px] text-gray-500 font-bold flex items-center gap-1.5"><TrendingUp className="w-3.5 h-3.5" /> {showBreakoutInfo ? "突破後目前漲幅" : "距離 Pivot 漲幅"}</span>
        ...
        <div className="bg-black/30 p-4 rounded-xl border border-slate-800 flex flex-col justify-center items-center text-center space-y-2 relative overflow-hidden">
            ...
            {stats.rule8WStatus === 'activated' ? (
                ...
                <>
                    <div className="flex items-center gap-2 text-indigo-400/60 font-black text-base">
                        <AlertCircle className="w-5 h-5" /> 規則尚未啟動
                    </div>
                    <p className="text-[10px] text-gray-500 leading-relaxed px-2">
                        尚未確認為強勢股。若突破後 3 週內漲幅達 20%，則此規則將啟動，屆時建議將持有時間拉長至 8 週。
                    </p>
                </>
            )}
        </div>
      ...
      <div className={`p-4 rounded-xl border flex items-center justify-between transition-all ${
        stats.strategy === 'hold' ? (stats.rule8WStatus === 'not_started' ? "bg-slate-500/10 border-slate-500/20" : "bg-emerald-500/10 border-emerald-500/20") :
        stats.strategy === 'reduce' ? "bg-amber-500/10 border-amber-500/20" :
        "bg-rose-500/10 border-rose-500/20"
      }`}>
        <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center border ${
                 stats.strategy === 'hold' ? (stats.rule8WStatus === 'not_started' ? "bg-slate-500/20 border-slate-500/30 text-slate-400" : "bg-emerald-500/20 border-emerald-500/30 text-emerald-400") :
                 stats.strategy === 'reduce' ? "bg-amber-500/20 border-amber-500/30 text-amber-400" :
                 "bg-rose-500/20 border-rose-500/30 text-rose-400"
            }`}>
                {stats.strategy === 'hold' ? (stats.rule8WStatus === 'not_started' ? <TrendingUp className="w-6 h-6" /> : <ShieldCheck className="w-6 h-6" />) : stats.strategy === 'reduce' ? <Info className="w-6 h-6" /> : <AlertCircle className="w-6 h-6" />}
            </div>
            <div>
                <h5 className="text-[10px] font-black uppercase tracking-widest text-gray-500 mb-0.5">持股管理建議</h5>
                <div className={`text-sm font-black ${
                     stats.strategy === 'hold' ? (stats.rule8WStatus === 'not_started' ? "text-slate-300" : "text-emerald-400") :
                     stats.strategy === 'reduce' ? "text-amber-400" :
                     "text-rose-400"
                }`}>
                    {stats.strategy === 'hold' ? (stats.rule8WStatus === 'not_started' ? "⚪ 初始持股/觀察" : "🟢 核心持有") : stats.strategy === 'reduce' ? "🟡 減碼觀察" : "🔴 出場"}
                </div>
            </div>
        </div>
        ...
        <div className="text-right hidden sm:block">
            <p className="text-[10px] text-gray-500 leading-tight max-w-[200px]">
                {stats.strategy === 'hold' ? (stats.rule8WStatus === 'not_started' ? "目前屬突破初期，規則尚未完全啟動且動能有待驗證，建議維持小規模持有並觀察。" : "目前價格仍在 Pivot、50MA 及停損點之上，且符合強勢股特徵，應耐心持有。") :
